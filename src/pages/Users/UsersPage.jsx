...
import { Loader2, AlertCircle, Plus, UserCircle } from "lucide-react";
...
const UsersPage = () => {
    ...
    const { data, isLoading, error, refetch } = useGet(["users"], "/api/admin/users");
    
    // Deletion State
    const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false);
    const [itemToDelete, setItemToDelete] = React.useState(null);

    const deleteMutation = useDelete("/api/admin/users", ["users"]);
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <PageHeader 
                ...
                actions={
                    <>
                        ...
                        >
                            <Plus className="mr-2 h-4 w-4" /> {t('add_new_user')}
                        </Button>
                    </>
                }
            />
            ...
            {!isLoading && !error && (
                <p className="text-[11px] text-slate-400 px-2 uppercase tracking-wider">
                    {t('total_records')}: <span className="font-bold text-slate-600">{usersData.length}</span>
                </p>
            )}
