...
const CityFormPage = () => {
  ...
  const postMutation = usePost("/api/admin/cities", ["cities"]);
  const updateMutation = useUpdate("/api/admin/cities", ["cities"]);
  ...
  React.useEffect(() => {
    if (isEditMode && response?.data?.city) {
  ...
  if (isEditMode && fetchError) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center text-center p-6">
        ...
        <Button variant="link" onClick={() => navigate("/cities")}>{t('back_to_list')}</Button>
      </div>
    );
  }
      ...
      >
        ...
        <div className="flex items-center justify-end gap-3 pt-6 border-t border-slate-50">
          ...
          >
            ...
            {isEditMode ? t('save_changes') : t('create_city')}
          </Button>
        </div>
      </form>
    </div>
