...
const AdminsPage = () => {
    ...
    const deleteMutation = useDelete("/api/admin/admins", ["admins"]);

    const handleEdit = (item) => {
        navigate(`/admins/edit/${item.id}`);
    };
    ...
    const columns = React.useMemo(
        () => getAdminsColumns(t, handleEdit, handleDeleteClick),
        [t]
    );
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 min-h-[300px] relative overflow-hidden">
                {isLoading ? (
                    ...
                    <div className="p-2">
                        <GenericDataTable columns={columns} data={adminsData} />
                    </div>
                )}
            </div>
            ...
            {!isLoading && !error && (
                <p className="text-[11px] text-slate-400 px-2 uppercase tracking-wider">
                    {t('total_records')}: <span className="font-bold text-slate-600">{adminsData.length}</span>
                </p>
            )}
