// src/pages/Garages/GaragesPage.jsx
import * as React from "react";
import { useNavigate } from "react-router-dom";
import { Loader2, AlertCircle, Warehouse } from "lucide-react";
import { Button } from "@/components/ui/button";
import { GenericDataTable } from "@/components/custom/GenericDataTable";
import { getGaragesColumns } from "./GaragesColumns";
...
const GaragesPage = () => {
    ...
    const moduleName = "pickup_points";
    const { data, isLoading, error, refetch } = useGet(["garages"], "/api/admin/garages");

    // Deletion State
    const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false);
    const [itemToDelete, setItemToDelete] = React.useState(null);

    const deleteMutation = useDelete("/api/admin/garages", ["garages"]);

    const handleEdit = (item) => {
        navigate(`/garages/edit/${item.id}`);
    };
    ...
    const garagesData = React.useMemo(() => {
        if (!data) return [];
        return data?.data?.garages || data?.garages || [];
    }, [data]);
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <PageHeader
                icon={Warehouse}
                title={t('garages')}
                subtitle={t('manage_garages')}
                addPath="/garages/add"
                addText={t('add_new_garage')}
            ...
            {!isLoading && !error && (
                <p className="text-[11px] text-slate-400 px-2 uppercase tracking-wider">
                    {t('total_records')}: <span className="font-bold text-slate-600">{garagesData.length}</span>
                </p>
            )}
