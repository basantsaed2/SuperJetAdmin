// src/pages/MaintenanceTypes/MaintenanceTypesPage.jsx
...
const MaintenanceTypesPage = () => {
    ...
    const { data, isLoading, error, refetch } = useGet(["maintenanceTypes"], "/api/admin/maintenanceTypes");
    
    // Deletion State
    const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false);
    const [itemToDelete, setItemToDelete] = React.useState(null);

    const deleteMutation = useDelete("/api/admin/maintenanceTypes", ["maintenanceTypes"]);

    const handleEdit = (item) => {
        navigate(`/maintenance_types/edit/${item.id}`);
    };
    ...
    const columns = React.useMemo(
        () => getMaintenanceTypeColumns(t),
        [t]
    );

    const maintenanceTypesData = React.useMemo(() => {
        if (!data) return [];
        return data?.data?.maintenanceTypes || data?.maintenanceTypes || [];
    }, [data]);
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <PageHeader 
                icon={Wrench}
                title={t('maintenance_types')}
                subtitle={t('manage_maintenance_types')}
                actions={
                    <>
                        <Button 
                            onClick={() => navigate("/maintenance_types/add")}
                            size="sm"
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 min-h-[300px] relative overflow-hidden">
                {isLoading ? (
                    ...
                    <div className="p-2">
                        <GenericDataTable columns={columns} data={maintenanceTypesData} onEdit={handleEdit} onDelete={handleDeleteClick} />
                    </div>
                )}
            </div>
            ...
            {!isLoading && !error && (
                <p className="text-[11px] text-slate-400 px-2 uppercase tracking-wider">
                    {t('total_records')}: <span className="font-bold text-slate-600">{maintenanceTypesData.length}</span>
                </p>
            )}
